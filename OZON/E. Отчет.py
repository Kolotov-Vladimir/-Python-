#Директор IT-корпорации оценивает эффективность работы сотрудников по различным показателям и критериям. Один из этих критериев сформулирован следующим образом: приступив к некоторому заданию, сотрудник должен завершить его, не переключаясь на другие задания.
#Чтобы проверить сотрудников на соответствие этому критерию, директор потребовал от каждого сотрудника отчет о том, какие задания он выполнял в последние n дней. Отчет — это последовательность из n целых чисел a1,a2,…,an, где ai — идентификатор задания, которое сотрудник выполнял в i-й день.
#Вам необходимо написать программу, проверяющую, соответствует ли сотрудник критерию по его отчету. Сотрудник соответствует этому критерию, если не существует такого задания x, которое выполнялось с перерывом (т. е. в некоторый день i сотрудник выполнял задание x, в дни с i+1 по j−1 он занимался другими заданиями, а в день j сотрудник продолжил выполнение задания x, при этом j>i+1). Иными словами, каждое задание, которое выполнял сотрудник, должно занимать один непрерывный отрезок дней.
#Неполные решения этой задачи (например, недостаточно эффективные) могут быть оценены частичным баллом.
#Входные данные
#В первой строке задано одно целое число t (1≤t≤10) — количество наборов входных данных.
#Каждый набор входных данных состоит из двух строк. В первой строке задано одно целое число n (3≤n≤50000). Во второй строке заданы n целых чисел a1,a2,…,an (1≤ai≤n) — отчет сотрудника.
#Выходные данные
#Для каждого набора входных данных выведите ответ на отдельной строке. Если отчет соответствует критерию, выведите YES, иначе выведите NO.

#Пример
#входные данные
#5
#5
#1 2 3 4 5
#4
#1 2 3 1
#8
#2 3 4 8 5 5 5 5
#5
#1 1 3 2 2
#5
#1 1 2 3 2
#выходные данные
#YES
#NO
#YES
#YES
#NO

#Решение
t = int(input()) # - количество наборов
for k in range(t): # - цикл из этих наборов
    n = int(input()) # - количество задач
    lst = [i for i in input().split() if i != ' '] #  - list (список), например, 1 2 3 4 1, заполняется внутренним циклом до тех,
    # пока не будет нажат enter ... различает значения с помощью пробела (' '), в итоге получится вот так lst = ['1', '2', '3', '4', '1']
    tr = True # - просто как count, некая переменная для итога Yes или No
    for i in range(n - 1): # - запускаю цикл по количеству задач, без последнего прохода (на самом деле можно просто n-1, без стартового 0)
        if lst.count(lst[i]) == 1: # - если текущее (например, 0-ое) значение из списка в единственном экземпляре, то пропускаем текущий цикл
            continue
        elif lst[i] == lst[i+1]: # - если текущее (например, 0-ое) значение из списка равно следующему значению из списка (например, 1 - ому), то пропускаем текущий цикл
            continue
        elif lst[i+1:].count(lst[i]) == 0: # - если после текущего значения i, из списка больше нет таких же значений, то пропускаем цикл ... тут получается я смотрю срез
            # lst[i+1:], т.е. если i = 1, то смотрим уже значения от 2 и до конца списка, если в срезе нет таких, то пропускаем
            continue
        else:
            ind = lst.index(lst[i], i, n) # тут я смотрю  индекс текущего значения из списка в срезе от текущего i
            ind2 = lst.index(lst[i], i+1, n) # тут я смотрю  индекс текущего значения из списка в срезе от текущего i + 1, то есть ищу индекс следующего текущего значения
            if ind == ind2 - 1: # вычитаю разность, и сли они стоят рядом, то ок, а если нет, то tr = False и прекращаю текущий цикл
                continue
            else:
                tr = False
                break
    if tr == True: # ну тут думаю понятно)
        print('YES')
    else:
        print('NO')

