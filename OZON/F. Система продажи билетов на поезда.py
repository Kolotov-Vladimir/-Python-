...
#
#Описание каждого набора начинается строкой, в которой записаны два целых числа n и q (1≤n≤3⋅10^5, 1≤m≤6⋅10^5) — количество купе в вагоне и количество запросов для обработки.
#
...
for i in range(t):
    pysto = input()
    tst = input().split(' ')
    kype_v_vagone = int(tst[0])
    zapros = int(tst[1])
    mesta = []
    for i in range(zapros):
        tst = input().split(' ')
        if int(tst[0]) == 1:
            if int(tst[1]) not in mesta:
                mesta.append(int(tst[1]))
                print('SUCCESS')
            else:
                print('FAIL')
        elif int(tst[0]) == 2:
            if int(tst[1]) in mesta:
                mesta.remove(int(tst[1]))
                print('SUCCESS')
            else:
                print('FAIL')
        elif int(tst[0]) == 3:
            tr = False
            for i in range(1, kype_v_vagone*2, 2):
                if i not in mesta and i + 1 not in mesta:
                    mesta.append(i)
                    mesta.append(i + 1)
                    print('SUCCESS ', i, '-', i+1, sep='')
                    tr = True
                    break
            if tr == False:
                print('FAIL')
