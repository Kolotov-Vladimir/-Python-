...
#
#Входные данные
#В первой строке задано целое число t (1≤t≤10^4) — количество наборов входных данных в тесте.
#
#Вторая строка содержит целое число n (1≤n≤2⋅10^5) — количество попыток регистрации в системе. Попытки заданы в хронологическом порядке (от самой ранней до самой поздней).
#
...
t = int(input())
abs = 'qwertyuiopasdfghjklzxcvbnm_-0123456789'
otv = []
for i in range(t):
    n = int(input())
    lst = []
    for j in range(n):
        tr = True
        m = input().lower()
        if len(m) < 2 or len(m) > 24 or m in lst or m[0] == '-':
            otv.append('NO')
            continue
