...
#
#Описание каждого набора начинается строкой, содержащей целое число n (1≤n≤10^5) — количество записей в журнале.
#
...
for i in range(t):
    ...
    for j in range(n):
        tst = input().split(' ')
        if tst[0] not in name:
            name.append(tst[0])
            name.sort()
            ind = name.index(tst[0])
            tel.insert(ind, [tst[1]])
        elif tst[0] in name:
            ind = name.index(tst[0])
            if tst[1] in tel[ind]:
                tel[ind].remove(tst[1])
                tel[ind].append(tst[1])
            elif len(tel[ind]) < 5:
                tel[ind].append(tst[1])
            elif len(tel[ind]) == 5:
                del tel[ind][0]
