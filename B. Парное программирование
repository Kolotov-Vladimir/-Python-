...
for _ in range(t):
    n = int(input())
    skill = [int(j) for j in input().split()]
    skill1 = [[i, skill[i]] for i in range(n)]
    i = 0
    teams = []
    for i in range(n // 2):
        diff = 99999999999
        place = 0
        for j in range(1, len(skill1)):
            if abs(skill1[0][1] - skill1[j][1]) < diff:
                diff = abs(skill1[0][1] - skill1[j][1])
                place = skill1[j]
        teams.append([skill1[0][0] + 1, place[0] + 1])
        skill1.pop(0)
        for k in range(len(skill1)):
            if skill1[k] == place:
                skill1.pop(k)
                break
    teams_finish.append(teams)
for team in teams_finish:
    for j in team:
        print(*j)
    print()
